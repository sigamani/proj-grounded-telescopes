...
try:
    from kyc_utils import (
        KYCRequest, KYCResponse, sanitize_kyc_input, 
        create_kyc_analysis_prompt, create_compliance_validation_prompt,
        validate_kyc_request, parse_kyc_response
    )
    ...
    def create_kyc_analysis_prompt(row):
        ...
        return {
            "messages": [
                ...
                {"role": "user", "content": f"Analyze company: {row.get('business_name', 'Unknown')}"}
            ],
...
def kyc_analyze_with_llm(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            analysis_json = {"error": "Invalid JSON response", "raw_response": analysis_text}
...
def kyc_compliance_validation(analysis_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            validation_json = {"error": "Invalid JSON response", "raw_response": validation_text}
...
def extract_kyc_metrics(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        validation = result.get("compliance_validation", {}).get("validation_result", {})
        
        return {
            "verdict": validation.get("final_verdict", "REVIEW"),
            "risk_score": analysis.get("risk_assessment", {}).get("overall_risk_score", "Medium"),
            "industry": analysis.get("industry_profile", {}).get("primary_industry", "Unknown"),
...
@weave.op()
def evaluate_kyc_prediction(expected: Dict[str, Any], actual: Dict[str, Any]) -> Dict[str, Any]:
    """
...
class KYCEvaluation(weave.Evaluation):
    ...
    @weave.op()
    def score(self, request_data: Dict[str, Any], prediction: Dict[str, Any]) -> Dict[str, Any]:
...
async def run_kyc_evaluation():
    ...
    evaluation_result = await evaluation.evaluate(
        dataset=UK_TEST_COMPANIES,
        scorers=[evaluate_kyc_prediction]
    )
    ...
    if scores:
        ...
        verdict_accuracy = sum(1 for s in scores if s.get("verdict_correct", False)) / len(scores)
        risk_accuracy = sum(1 for s in scores if s.get("risk_correct", False)) / len(scores)
        industry_accuracy = sum(1 for s in scores if s.get("industry_correct", False)) / len(scores)
        avg_overall_score = sum(s.get("overall_score", 0) for s in scores) / len(scores)
        avg_processing_time = sum(s.get("processing_time", 0) for s in scores) / len(scores)
        ...
        for i, (company, score) in enumerate(zip(UK_TEST_COMPANIES, scores)):
            print(f"{i+1}. {company['business_name']}: {score.get('overall_score', 0):.3f}")
            if not score.get('verdict_correct', True):
                print(f"   ❌ Verdict: Expected {company['expected_verdict']}, Got {score.get('actual_verdict', 'N/A')}")
            if not score.get('risk_correct', True):
                print(f"   ❌ Risk: Expected {company['expected_risk']}, Got {score.get('actual_risk', 'N/A')}")
