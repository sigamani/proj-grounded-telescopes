...
class TestFullPipeline:
    ...
    def test_monitoring_stack_accessibility(self, docker_compose_stack):
        ...
        for url, service in monitoring_endpoints:
            try:
                ...
                assert response.status_code in [200, 302, 401], f"{service} not accessible"
            ...
            if response.status_code == 200:
                ...
                while elapsed < max_wait:
                    status_response = requests.get(
                        f"http://localhost:8265/api/jobs/{job_id}",
                        timeout=10
                    )
    ...
    def test_volume_persistence(self, docker_compose_stack):
        ...
        for vol_name in expected_volumes:
            assert any(vol_name in name for name in volume_names), f"Volume {vol_name} not found"
