...
try:
    from kyc_weave import (
        UK_TEST_COMPANIES, KYCEvaluationResult,
        kyc_analyze_with_llm, kyc_compliance_validation,
        process_kyc_request_traced, extract_kyc_metrics,
        evaluate_kyc_prediction, KYCEvaluation
    )
...
class TestUKDataset:
    ...
    def test_uk_dataset_structure(self):
        ...
        required_fields = [
            "business_name", "address", "country_code", "registration_id", 
            "website_url", "expected_verdict", "expected_risk", "expected_industry"
        ]
        
        for company in UK_TEST_COMPANIES:
            for field in required_fields:
                assert field in company, f"Missing {field} in {company.get('business_name', 'Unknown')}"
...
class TestKYCEvaluationModels:
    ...
    def test_kyc_evaluation_result_scoring(self):
        ...
        result = KYCEvaluationResult(
            company_name="Test",
            actual_verdict="ACCEPT", expected_verdict="REJECT",
            actual_risk="Low", expected_risk="High",
            actual_industry="Tech", expected_industry="Finance",
        ...
        with pytest.raises(ValueError):
            KYCEvaluationResult(
                company_name="Test",
                actual_verdict="ACCEPT", expected_verdict="ACCEPT",
                actual_risk="Low", expected_risk="Low", 
                actual_industry="Tech", expected_industry="Tech",
                verdict_correct=True, risk_correct=True, industry_correct=True,
...
class TestKYCWeaveOps:
    ...
    def test_process_kyc_request_traced(self, mock_sanitize, mock_validate, 
                                       mock_analyze, mock_compliance):
...
class TestKYCIntegrationWithWeave:
    ...
    def test_uk_dataset_integration(self):
        ...
        mock_result = {
            "initial_analysis": {
                "analysis_result": {
                    "industry_profile": {"primary_industry": sample_company["expected_industry"]}
                }
            },
            "compliance_validation": {
                "validation_result": {"final_verdict": sample_company["expected_verdict"]}
            },
            "processing_time": 2.0
        }
